...
export const SHARED_OVERRIDES = new Set([
  ...
  /*
   * The block discriminator. `type` is in LOCALIZED_KEYS because it genuinely is
   * copy at logos.items[].type ("Agreement", "Client") — but at the ROOT of a
   * block it is `type: "hero"`, the thing that decides which renderer runs.
   *
   * Storing that per locale meant the identity of every block was duplicated
   * across two rows, free to diverge, in a schema whose entire point is that
   * structure lives once on a locale-free row. The locale-invariance audit could
   * not see it: "hero" is "hero" in Arabic too, so both halves matched.
   *
   * Overrides are keyed `parentKey.key`, and a root-level key has no parent, so
   * bare `type` resolves here while `items.type` keeps falling through to
   * LOCALIZED_KEYS. Caught by building the editor: `type` appeared as an
   * editable text field on hero, section, pageTitle, careers and socialStrip.
   */
  'type',
]);
